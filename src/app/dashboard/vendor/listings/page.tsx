  ...
  const handleDelete = async (vehicleId: string) => {
    if (
      ...
      try {
        ...
        // <-- PERBAIKAN 2: Gunakan 'unknown'
        if (err instanceof Error) {
          toast.error(err.message, { id: toastId });
        } else {
          toast.error("Gagal menghapus listing", { id: toastId });
        }
      }
    }
  };
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center bg-gradient-to-r from-teal-500 to-teal-600 text-white px-6 py-3 rounded-xl hover:from-teal-600 hover:to-teal-700 shadow-lg hover:shadow-xl transition-all duration-200 font-medium"
              >
        ...
        {listings.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12">
            <div className="text-center max-w-md mx-auto">
              ...
              <Link
                href="/dashboard/vendor/listings/new"
                className="inline-flex items-center bg-gradient-to-r from-teal-500 to-teal-600 text-white px-8 py-4 rounded-xl hover:from-teal-600 hover:to-teal-700 shadow-lg hover:shadow-xl transition-all duration-200 font-medium"
              >
                <PlusCircle className="w-5 h-5 mr-2" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {listings.map((vehicle) => (
              <div key={vehicle.id} className="relative group">
                {/* Kita bungkus VehicleCard di dalam div agar bisa menambahkan tombol aksi */}
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300">
                  <VehicleCard vehicle={vehicle} />
                </div>

                {/* Action Buttons Overlay (Hanya satu set tombol) */}
