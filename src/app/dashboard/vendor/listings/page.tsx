    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b pb-4 gap-4">
        <div>
          ...
          {/* PERBAIKAN DI SINI: Menggunakan <div>, bukan <p> */}
        ...
        <Link
          href="/dashboard/vendor/listings/new"
          className="inline-flex items-center bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 shadow-sm transition-colors whitespace-nowrap"
        >
          <PlusCircle className="w-5 h-5 mr-2" />
          Tambah Kendaraan
        </Link>
      ...
      {isLoading ? (
        <div className="text-center py-20">Memuat listing Anda...</div>
      ) : error ? (
          ...
          {listings.map((vehicle) => (
            <div key={vehicle.id} className="relative group">
              ...
              <div className="absolute top-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link
                  href={`/dashboard/vendor/listings/${vehicle.id}/edit`}
                  className="p-2 bg-white/80 backdrop-blur-sm text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded-full shadow transition-colors"
                  title="Edit Listing"
                >
                  <Edit className="w-4 h-4" />
                </Link>
                <button
                  onClick={() => handleDelete(vehicle.id)}
                  className="p-2 bg-white/80 backdrop-blur-sm text-red-600 hover:text-red-800 hover:bg-red-100 rounded-full shadow transition-colors"
                  title="Hapus Listing"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
